  ...
  async getTimeline(branchId: string): Promise<readonly TimelineEvent[]> {
    ...
    for (;;) {
      const page = await this.#request<ApiPage<TimelineEvent>>(
        `/branches/${encodeURIComponent(branchId)}/timeline?fromSeq=${String(fromSeq)}&limit=500`,
      );
      all.push(...page.items);
...
const styles = `
  ...
  * { box-sizing:border-box; }
  ...
  @media (max-width:860px) { .masthead { grid-template-columns:1fr auto; }.status { grid-column:1/-1; grid-row:2; }.controls { grid-template-columns:1fr 1fr; padding:14px 0; }.scrub { grid-column:1/-1; }.workspace { grid-template-columns:1fr; }.timeline-panel { border-right:0; padding-right:0; }.inspector { padding-left:44px; }.timeline { max-height:55vh; } }
